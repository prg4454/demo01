...
@Component({
    ...
    templateUrl: './dogs-entry-modal.component.html',
    styleUrl: './dogs-entry-modal.component.scss'
})
export class DogsEntryModalComponent implements OnInit {
    ...
    @Input({ required: true }) dog!: DogRecord;
    @Input() allowDelete = false;

    editDraft: DogRecord | null = null;
    originalDraft: DogRecord | null = null;
    saveAttempted = false;
    readonly statuses: DogRecord['status'][] = ['Waiting', 'Exam', 'Treatment', 'Ready'];

    ngOnInit(): void {
        this.editDraft = structuredClone(this.dog);
        this.originalDraft = structuredClone(this.dog);
    }
    ...
    async requestDelete(): Promise<void> {
        ...
        this.activeModal.close({ action: 'delete', dog: this.editDraft } satisfies DogsModalResult);
    }
